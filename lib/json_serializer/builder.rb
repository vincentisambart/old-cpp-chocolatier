...
module JSONSerializer
  module Builder
    ...
    def self.llvm_version_used_by_executable
      return nil unless BINARY_PATH.exist?
      output = `#{BINARY_PATH.to_s.shellescape} -version`.strip
      raise "Error running #{json_serializer_binary_path}" unless $?.success?
      match_data = /LLVM version ([0-9.]+)/.match(output)
      raise "Could not find LLVM version used to build #{BINARY_PATH}" unless match_data
      match_data[1]
    end
    ...
    def self.up_to_date?
      ...
      SOURCE_PATH.mtime <= BINARY_PATH.mtime
    end
    ...
    def self.build
      ...
      command = "clang++ -o #{BINARY_PATH.to_s.shellescape} -g -O2 #{cxxflags} -I#{include_dir.to_s.shellescape} #{ldflags} #{libs} #{system_libs} #{clang_libs} -stdlib=libc++ -std=c++14 #{SOURCE_PATH.to_s.shellescape}"
